...
[From the iotnexus github repo](https://github.com/iotnexuscoin/iotnexus/blob/master/doc/masternode_conf.md)
...
## What about the iotnexus.conf file?

If you are using a `masternode.conf` file you no longer need the `iotnexus.conf` file. The exception is if you need custom settings (_thanks oblox_). In that case you **must** remove `masternode=1` from local `iotnexus.conf` file. This option should be used only to start local Hot masternode now.

## Update iotnexus.conf on server

If you generated a new masternode private key, you will need to update the remote `iotnexus.conf` files.

Shut down the daemon and then edit the file.

```nano .iotnexuscore/iotnexus.conf```

### Edit the masternodeprivkey
If you generated a new masternode private key, you will need to update the `masternodeprivkey` value in your remote `iotnexus.conf` file.
...
```iotnexus-cli getinfo```

and comparing with the official explorer at https://explorer.iotnexus.org/chain/IoTNexus
...
```
iotnexus-cli masternode status
{
...
Search your Masternodes on https://iotnexusninja.pl/masternodes.html
