...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("IOTNMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("IOTNMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("IOTNMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("iotnexus-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("IOTNMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running IOTNMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadIOTNMiner() exception");
    } catch (...) {
        LogPrintf("ThreadIOTNMiner() exception");
    }

    LogPrintf("ThreadIOTNMiner exiting\n");
}
